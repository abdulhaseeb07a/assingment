function getDistinctApplicationTypes(arr){
    ...
    const results = uniqueValues.filter(element => {
      return element !== null;
    });
    return results
  }
  ...
  function getFilteredData(logs,params){
    ...
    if(params.apType  != undefined){
      filteredData = filteredData.filter(auditLog =>( auditLog.applicationType === params.apType))
    }
    if(params.acType  != undefined){
      filteredData = filteredData.filter(auditLog =>( auditLog.actionType === params.acType))
    }
    if(params.apId  != undefined){
      filteredData = filteredData.filter(auditLog =>( auditLog.applicationId == params.apId))
    }
    if(params.fDate!=undefined && params.tDate!= undefined){
      ...
      filteredData = filteredData.filter(function (a) {
          ...
          return hitDates.getTime() >= startDate.getTime() && hitDates.getTime() <= endDate.getTime()
      });
    }
    return filteredData;
  }
